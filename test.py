...
#testing
#mnist_type=['original', 'rotated', 'extended', 'affnist']
mnist_type = ['cht_mnist', 'cvt_mnist', 'cdt_mnist']
#_, _, _, _, ts_x, ts_y, _ = util.load_mnist(mnist_type='extended', onehotencoder=False)
#plot_images(ts_x, ts_y, num_row=2, num_col=5)

for t in mnist_type:
    x_train, y_train, _, _, x_test, y_test, _ = dataset_loader.load_mnist(mnist_type=t, onehotencoder=False)
